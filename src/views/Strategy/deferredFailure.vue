<!-- 递延失败 -->
<template>
<div id="deferredFailure" style="position:relative">
	<div class="clearfloat">
		<Button type="primary" class="left" @click='handleSearch(1)'>查询</Button>
		<Button type="primary" class="right" @click='handleSendMsg'>发送提示信息</Button>
...
export default {
	...
	methods: {
		...
		getData(para) {
			...
			let api = this.$api.strategy.policies_auto_hold_failed
			this.$axios.get(api, {
				params: para
			}).then(res => {
		...
		handleSendMsg() {
			let api = this.$api.strategy.policies_auto_hold_failed_push
			this.$Modal.confirm({
				content: '<p>确认发送信息!?</p>',
				okText: '确定',
				cancelText: '取消',
