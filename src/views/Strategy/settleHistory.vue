...
<div id="settlement" style="position:relative">
	...
	<!-- 搜索 -->
	<div class="clearfloat">
		<Select class="input left" clearable v-model="fillter.user_id" placeholder="策略人/手机号" :loading='userLoading' filterable remote :remote-method="remoteMethodUser">
			<Option v-for='item in userList' :key='item.id' :value='item.id'>{{item.name}}</Option>
		</Select>
		<Select class="input left" v-model="fillter.agent_id" clearable placeholder="代理商名称" :loading='agentLoading' filterable remote :remote-method="remoteMethodAgent">
			<Option v-for='item in agentList' :key='item.id' :value='item.id'>{{item.name}}</Option>
		</Select>
		<Select class="input left" v-model="fillter.stock_id" clearable placeholder="股票代码" :loading='stockLoading' filterable remote :remote-method="remoteMethodStock">
			<Option v-for='item in stockList' :key='item.id' :value='item.id'>{{item.stock_name}}</Option>
		</Select>
		<Select class="input left" v-model="fillter.trade_sell_type" clearable placeholder="平仓状态" filterable>
			<Option v-for='item in statusList' :key='item.value' :value='item.value'>{{item.label}}</Option>
		</Select>
		<Col span="12" class="input">
		<DatePicker type="daterange" v-model='fillter.time' placement="bottom-end" placeholder="结算时间" style="width: 200px"></DatePicker>
		</Col>
	</div>
	<!-- 表格 -->
...
export default {
	...
	methods: {
		...
		getData(para) {
			...
			let api = this.$api.strategy.policies_settlement_history
			this.$axios.get(api, {
				params: para
			}).then(res => {
		...
		handleInfo(row) {
			this.dialog = true
			this.spinShow = true
			let api = this.$api.strategy.policy_settlement_history
			let para = {
				id: row.id
			}
			this.$axios.get(api, {
		...
		handleSearch(count) {
			var start_time = ''
			var end_time = ''
			if (this.fillter.time[0] != '') {
				start_time = this.handletimer(this.fillter.time[0])
			}
			if (this.fillter.time[1] != '') {
				end_time = this.handletimer(this.fillter.time[1])
			}
			if (count == 1) {
				this.page = 1
			}
			let para = {
				trade_sell_type: this.fillter.trade_sell_type,
				user_id: this.fillter.user_id,
				end_time: end_time,
				start_time: start_time,
				stock_id: this.fillter.stock_id,
				agent_id: this.fillter.agent_id,
				limit: this.limit,
				page: this.page
			}
			this.getData(para)
		...
		// 导出
		handleExport() {
			let api = this.$api.common.export_policies_settlement_history
			this.$Modal.confirm({
				...
				onOk: () => {
					this.$axios.get(api, {
						...
						let fileName = new Date() + 'settlement_history.xlsx'
