...
<div id="deal" style="position:relative">
	<div class="clearfloat">
		<Button type="primary" class="left" @click='handleSearch(0)'>查询</Button>
	...
	<div class="clearfloat">
		...
		<DatePicker type="daterange" v-model='filter.time' placement="bottom-end" placeholder="成交时间" style="width: 200px"></DatePicker>
		</Col>
	</div>
	<!-- 表格 -->
...
export default {
	...
	methods: {
		...
		getData(para) {
			...
			let api = this.$api.strategy.policies_deal
			this.$axios.get(api, {
				params: para
			}).then(res => {
		...
		// 导出
		handleExport() {
			let api = this.$api.common.export_policies_deal
			this.$Modal.confirm({
				...
				onOk: () => {
					this.$axios.get(api, {
						...
						let fileName = new Date() + 'deal.xlsx'
