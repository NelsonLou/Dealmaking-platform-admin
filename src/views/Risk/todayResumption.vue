<!-- 今日复牌 -->
<template>
<div id="todayResumption" style="position:relative">
	<div class="" v-show='!addDialog'>
		...
		<!-- 搜索 -->
		<div class="clearfloat">
			<Input class="input left" v-model="filter.stock_select" placeholder="股票代码/名称"></Input>
	...
	<div class="" v-show='addDialog'>
		<div class="clearfloat">
			<Select class="input left" v-model="formfilter.stock_id" clearable placeholder="股票代码" :loading='stockLoading' :label-in-value='true' filterable remote :remote-method="remoteMethodStock">
...
export default {
		...
		return {
			...
			columns: [{
				...
				},
				{
					...
					render: (h, params) => {
						return h("div", [
							h("div", this.handleDate(params.row.start_time)),
						])
					}
				},
				{
	...
	methods: {
		...
		getData(para) {
			...
			let api = this.$api.risk.start_stocks
			this.$axios.get(api, {
				params: para
			}).then(res => {
